...
import '../../pages/course_details/enroll_course_new/enroll_course_screen.dart';
...
//  late VideoPlayerController videoPlayerController;
  ...
  checkCartData(CheckCartType checkCartType) async {
    try {
      ...
      print(chheck_CartResponse.cartdata?.toJson());
      switch ((chheck_CartResponse).success) {
        case true:
          if (chheck_CartResponse.cartdata?.cart != null) {
            switch (checkCartType) {
              case CheckCartType.check:
                print("heeereeee3");
                deleteCart(
                    (chheck_CartResponse.cartdata?.cart?.items![0].id ?? 0).toString());
                break;
              case CheckCartType.setData:
                Get.to(EnrollCourseScreen(courseID,courseDetails.value?.price));
                courseToBeEnrolled =
                    chheck_CartResponse.cartdata?.cart?.items![0].webinar;
                // Get.to(EnrollCourseScreen(courseToBeEnrolled!));
  ...
  checkoutCourse(String coupon, String discountID,String subTotal,String discount,String total) async {
    try {
      final _checkoutCourseResponse =
          await _courseDetailsUseCase.checkoutCourse(coupon, discountID,subTotal,discount,total);
      ...
      switch ((_checkoutCourseResponse).success) {
        case true:
          ...
          showToast(_checkoutCourseResponse.message.toString(), gravity: Toast.bottom);
          //Get.back();
          break;
        case false:
          dismissLoading();
    ...
    try {
      ...
      switch ((_validateCourseResponse).success) {
        case true:
          ...
          discount.value=item!.amounts!.totalDiscount!;
          totalAfterDiscount.value=_validateCourseResponse.couponData!.amounts!.total!;
          discountId.value=item!.discount!.id!.toString();
