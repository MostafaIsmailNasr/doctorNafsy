...
class HomeController extends BaseController {
  ...
  var homeListResponse = HomeListResponse().obs;
  RxList<dynamic> doctorsList=[].obs;
  RxList<dynamic> coachsList=[].obs;
  RxList<dynamic> therapistsList=[].obs;
  ///
...
// use the returned token to send messages to users from your custom server
    ...
    try {
      ...
      switch ((profileResponse as ProfileResponse).success) {
        case true:
          ...
          if (( store.user?.status==("active")&&  profileResponse.userData?.user?.deviceId==(store.user?.deviceId))){
              //||( store.user?.status==("active")&& store.user?.mobile==("01044444478"))
          ...
          // if (( store.user?.status==("active")&&  store.user?.deviceId==(androidId))) {
          //   print("=== rrrr");
          // } else {
          //   logout(context);
          // }
  ...
  getHomeNewList()async{
    final storage = Get.find<LocalStorageService>();
    //isLoading.value=true;
    homeListResponse.value = await repo.getHomeNewList();
    doctorsList.value=homeListResponse.value.data?.doctors??[] as List;
    coachsList.value=homeListResponse.value.data?.coachs??[];
    therapistsList.value=homeListResponse.value.data?.therapists??[];
    isLoading.value=false;
    return homeListResponse.value;
  }
